...
describe("AdReferralService", () => {
  class FakeLocalStorage {
    constructor(private readonly items: { [key: string]: string } = {}) {}
  ...
  test("returns all UTM params", () => {
    const window = makeWindow(
      "?utm_source=foo&utm_campaign=bar&utm_medium=mux&utm_content=clob"
    );
    const service = makeService(window);
  ...
  test("Does not return expired data", () => {
    ...
    {
      const window = makeWindow("?utm_source=foo", localStorage);
    ...
    // Test data is returned when almost expired.
    {
      ...
      const service = makeService(window, () => TIME + EXPIRES_MS - 1000);
      service.onAppInit();
